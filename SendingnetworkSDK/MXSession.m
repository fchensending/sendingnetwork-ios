...
NSString *const kMXSessionNoRoomTag = @"m.recent";  // Use the same value as sendingnetwork-react-sdk
...
@synthesize sendingnetworkRestClient, mediaManager;

- (id)initWithSendingnetworkRestClient:(MXRestClient*)mxRestClient
{
    self = [super init];
    if (self)
    {
        sendingnetworkRestClient = mxRestClient;
        _threePidAddManager = [[MX3PidAddManager alloc] initWithSendingnetworkSession:self];
        mediaManager = [[MXMediaManager alloc] initWithHomeServer:sendingnetworkRestClient.homeserver];
        ...
        _notificationCenter = [[MXNotificationCenter alloc] initWithSendingnetworkSession:self];
...
{
    return sendingnetworkRestClient.credentials;
}

- (NSString *)myUserId
{
    return sendingnetworkRestClient.credentials.userId;
}

- (NSString *)myDeviceId
{
    return sendingnetworkRestClient.credentials.deviceId;
}
...
-(void)setStore:(id<MXStore>)store success:(void (^)(void))onStoreDataReady failure:(void (^)(NSError *))failure
{
    ...
    _storeService = [[MXStoreService alloc] initWithStore:store credentials:sendingnetworkRestClient.credentials];
    ...
    MXWeakify(self);
    [self.store openWithCredentials:sendingnetworkRestClient.credentials onComplete:^{
        MXStrongifyAndReturnIfNil(self);
        
        // Sanity check: The session may be closed before the end of store opening.
        if (!self->sendingnetworkRestClient)
        {
            return;
        }

        self.storeService.aggregations = [[MXAggregations alloc] initWithSendingnetworkSession:self];

        // Check if the user has enabled crypto
        MXWeakify(self);
        [MXCrypto checkCryptoWithSendingnetworkSession:self complete:^(MXCrypto *crypto) {
            MXStrongifyAndReturnIfNil(self);
            ...
            if (self.store.isPermanent && self.isEventStreamInitialised)
            {
                ...
                MXUser *myUser = [self.store userWithUserId:self->sendingnetworkRestClient.credentials.userId];
            ...
            else
            {
                ...
                self->_myUser = [[MXMyUser alloc] initWithUserId:self->sendingnetworkRestClient.credentials.userId];
                self->_myUser.mxSession = self;
...
- (void)setIdentityServer:(NSString *)identityServer andAccessToken:(NSString *)accessToken
{
    ...
    sendingnetworkRestClient.identityServer = identityServer;

    if (identityServer)
    {
        _identityService = [[MXIdentityService alloc] initWithIdentityServer:identityServer accessToken:accessToken andHomeserverRestClient:sendingnetworkRestClient];
    ...
    MXWeakify(self);
    sendingnetworkRestClient.identityServerAccessTokenHandler = ^MXHTTPOperation *(void (^success)(NSString *accessToken), void (^failure)(NSError *error)) {
...
- (void)_startWithSyncFilterId:(NSString *)syncFilterId onServerSyncDone:(void (^)(void))onServerSyncDone failure:(void (^)(NSError *))failure
{
    ...
    else
    {
        ...
        MXWeakify(self);
        [_myUser updateFromHomeserverOfSendingnetworkSession:self success:^{
            MXStrongifyAndReturnIfNil(self);
    ...
    // Refresh supported Sendingnetwork versions
    [self refreshSupportedSendingnetworkVersions:nil failure:nil];
    
    // Get the maximum file size allowed for uploading media
    [self.sendingnetworkRestClient maxUploadSize:^(NSInteger maxUploadSize) {
                   ...
                   failure:(void (^)(NSError *error))failure
{
    ...
    [self enableCrypto:NO success:^{
        ...
        {
            MXHTTPOperation *operation2 = [self.sendingnetworkRestClient logout:success failure:failure];
...
- (MXHTTPOperation*)deactivateAccountWithAuthParameters:(NSDictionary*)authParameters
                                           eraseAccount:(BOOL)eraseAccount
                                                ...
                                                failure:(void (^)(NSError *error))failure
{
    return [self.sendingnetworkRestClient deactivateAccountWithAuthParameters:authParameters
                        ...
                        setPresence:(NSString*)setPresence
{
    ...
    else
    {
        ...
        MXWeakify(self);
        eventStreamRequest = [sendingnetworkRestClient syncFromToken:streamToken serverTimeout:serverTimeout clientTimeout:clientTimeout setPresence:setPresence filter:self.syncFilterId success:^(MXSyncResponse *liveResponse) {
            MXStrongifyAndReturnIfNil(self);
        ...
        else
        {
            ...
            else
            {
                ...
                if (networkReachabilityManager.isReachable)
                {
                    ...
                    // Random time it used to avoid all Sendingnetwork clients to retry all in the same time
...
- (void)handlePresenceEvent:(MXEvent *)event direction:(MXTimelineDirection)direction
{
    ...
    if (userId)
    {
        ...
        [user updateWithPresenceEvent:event inSendingnetworkSession:self];
...
//                                                                          userInfo:nil];
            ...
            if([event[@"type"] isEqualToString:kMXAccountDataTypeRemarkUserList]) {
                [ContactService.sharedInstance fetchRemarkList];
            }
        }
...
- (void)enableVoIPWithCallStack:(id<MXCallStack>)callStack
{
    ...
    _callManager = [[MXCallManager alloc] initWithSendingnetworkSession:self andCallStack:callStack];
}
...
- (void)enableCrypto:(BOOL)enableCrypto success:(void (^)(void))success failure:(void (^)(NSError *))failure
{
    ...
    {
        _crypto = [MXCrypto createCryptoWithSendingnetworkSession:self];
...
- (void)setAntivirusServerURL:(NSString *)antivirusServerURL
{
    ...
    [sendingnetworkRestClient setAntivirusServer:antivirusServerURL];
    
    // Configure scan manager if antivirusServerURL is set
    if (antivirusServerURL)
    {
        _scanManager = [[MXScanManager alloc] initWithRestClient:sendingnetworkRestClient];
...
- (MXHTTPOperation*)createRoom:(NSString*)name
                    visibility:(MXRoomDirectoryVisibility)visibility
                     roomAlias:(NSString*)roomAlias
                       ...
                       failure:(void (^)(NSError *error))failure
{
    return [sendingnetworkRestClient createRoom:name visibility:visibility roomAlias:roomAlias topic:topic success:^(MXCreateRoomResponse *response) {
...
- (MXHTTPOperation*)createRoomWithParameters:(MXRoomCreationParameters*)parameters
                                     ...
                                     failure:(void (^)(NSError *error))failure
{
    return [sendingnetworkRestClient createRoomWithParameters:parameters success:^(MXCreateRoomResponse *response) {
...
- (MXHTTPOperation*)createRoom:(NSDictionary*)parameters
                       ...
                       failure:(void (^)(NSError *error))failure
{
    return [sendingnetworkRestClient createRoom:parameters success:^(MXCreateRoomResponse *response) {
...
- (MXHTTPOperation*)joinRoom:(NSString*)roomIdOrAlias
    ...
    return [sendingnetworkRestClient joinRoom:roomIdOrAlias viaServers:viaServers withThirdPartySigned:nil success:^(NSString *theRoomId) {
                      ...
                      failure:(void (^)(NSError *error))failure
{
    ...
    return [sendingnetworkRestClient leaveRoom:roomId success:^{
...
{
    MXRoom *room = nil;
    if ([MXTools isSendingnetworkRoomIdentifier:roomIdOrAlias])
    {
        room = [self roomWithRoomId:roomIdOrAlias];
    }
    else if ([MXTools isSendingnetworkRoomAlias:roomIdOrAlias])
    {
...
- (MXRoom *)directDeletedRoomWithUserId:(NSString*)userId
{
//    // Retrieve the existing direct chats
//    NSArray *directRoomIds = self.directRooms[userId];
//    
...
//        {
//            // Check whether the user membership is joined
//            if (directRoom.summary.membership == MXMembershipLeave)
//            {
//                return directRoom;
//            }
//        }
...
{
    MXRoom *room = [[MXRoom alloc] initWithRoomId:roomId andSendingnetworkSession:self];
...
{
    MXRoomSummary *summary = [[MXRoomSummary alloc] initWithRoomId:roomId andSendingnetworkSession:self];
...
{
    MXRoom *room = [MXRoom loadRoomFromStore:self.store withRoomId:roomId sendingnetworkSession:self];
...
#pragma mark - Sendingnetwork Events
                             ...
                             failure:(void (^)(NSError *error))failure
{
    ...
    if (roomId)
    {
        ...
        {
            operation = [sendingnetworkRestClient eventWithEventId:eventId inRoom:roomId success:decryptIfNeeded failure:failure];
        }
    }
    else
    {
        operation = [sendingnetworkRestClient eventWithEventId:eventId success:decryptIfNeeded failure:failure];
    }
    ...
    if (roomId)
    {
        ...
        if (roomSummary == nil)
        {
            ...
            if (roomSummaryProtocol)
            {
                ...
                [roomSummary setSendingnetworkSession:self];
                              ...
                              failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sendingnetworkRestClient acceptGroupInvite:groupId success:^{
        MXStrongifyAndReturnIfNil(self);
                       ...
                       failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sendingnetworkRestClient leaveGroup:groupId success:^{
        MXStrongifyAndReturnIfNil(self);
                                 ...
                                 failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sendingnetworkRestClient updateGroupPublicity:group.groupId isPublicised:isPublicised success:^(void) {
        MXStrongifyAndReturnIfNil(self);
        ...
        [self->userIdsWithOutdatedPublicisedGroups addObject:self->sendingnetworkRestClient.credentials.userId];
        [self publicisedGroupsForUser:self->sendingnetworkRestClient.credentials.userId];
                            ...
                            failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sendingnetworkRestClient getGroupProfile:group.groupId success:^(MXGroupProfile *groupProfile) {
        MXStrongifyAndReturnIfNil(self);
                            ...
                            failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sendingnetworkRestClient getGroupSummary:group.groupId success:^(MXGroupSummary *groupSummary) {
        MXStrongifyAndReturnIfNil(self);
                          ...
                          failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sendingnetworkRestClient getGroupUsers:group.groupId success:^(MXGroupUsers *groupUsers) {
        MXStrongifyAndReturnIfNil(self);
                                    ...
                                    failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sendingnetworkRestClient getGroupInvitedUsers:group.groupId success:^(MXGroupUsers *invitedUsers) {
        MXStrongifyAndReturnIfNil(self);
                          ...
                          failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    return [sendingnetworkRestClient getGroupRooms:group.groupId success:^(MXGroupRooms *groupRooms) {
        MXStrongifyAndReturnIfNil(self);
...
- (void)peekInRoomWithRoomId:(NSString*)roomId
                     ...
                     failure:(void (^)(NSError *error))failure
{
    MXPeekingRoom *peekingRoom = [[MXPeekingRoom alloc] initWithRoomId:roomId andSendingnetworkSession:self];
...
#pragma mark - Sendingnetwork users
...
{
    // Algo (and the [0.0, 1.0] assumption) inspired from sendingnetwork-react-sdk:
...
{
    MXWeakify(self);
    return [sendingnetworkRestClient setAccountData:data forType:type success:^{
        MXStrongifyAndReturnIfNil(self);
                                          ...
                                          failure:(void (^)(NSError *))failure
{
    ...
    if (identityServer)
    {
        ...
        __block MXIdentityService *identityService = [[MXIdentityService alloc] initWithIdentityServer:identityServer accessToken:nil andHomeserverRestClient:sendingnetworkRestClient];
                                          ...
                                          failure:(void (^)(NSError *error))failure
{
    ...
    if (!identityService)
    {
        ...
        identityService = [[MXIdentityService alloc] initWithIdentityServer:baseURL
                                                                accessToken:nil
                                                    andHomeserverRestClient:self.sendingnetworkRestClient];
    }
                                        ...
                                        failure:(void (^)(NSError *))failure
{
    ...
    if (!autoDiscovery)
    {
        ...
        if (!homeServer)
        {
            ...
            NSString *userDomain = [MXTools serverNameInSendingnetworkIdentifier:self.myUserId];
            ...
            {
                homeServer = sendingnetworkRestClient.homeserver;
            }
        }
                                           ...
                                           failure:(void (^)(NSError *))failure
{
    ...
    MXWeakify(self);
    return [self.sendingnetworkRestClient capabilities:^(MXCapabilities *capabilities) {
        MXStrongifyAndReturnIfNil(self);
...
#pragma mark - Supported Sendingnetwork versions

- (MXHTTPOperation*)supportedSendingnetworkVersions:(void (^)(MXSendingnetworkVersions *))success failure:(void (^)(NSError *))failure
{
    MXSendingnetworkVersions *supportedVersionsInStore = self.store.supportedSendingnetworkVersions;
    ...
    return [sendingnetworkRestClient supportedSendingnetworkVersions:success failure:failure];
}

- (MXHTTPOperation *)refreshSupportedSendingnetworkVersions:(void (^)(MXSendingnetworkVersions *))success
                                            failure:(void (^)(NSError *))failure
{
    MXLogDebug(@"[MXSession] refreshSendingnetworkSupportedVersions");

    MXWeakify(self);
    return [self.sendingnetworkRestClient supportedSendingnetworkVersions:^(MXSendingnetworkVersions *sendingnetworkVersions) {
        MXStrongifyAndReturnIfNil(self);

        if (sendingnetworkVersions)
        {
            [self.store storeSupportedSendingnetworkVersions:sendingnetworkVersions];
        }
...
#pragma mark - Sendingnetwork filters
                      ...
                      failure:(void (^)(NSError *error))failure
{
    ...
    if (self.store)
    {
        ...
        [self.store filterIdForFilter:filter success:^(NSString * _Nullable filterId) {
            ...
            else
            {
                ...
                MXWeakify(self);
                MXHTTPOperation *operation2 = [self.sendingnetworkRestClient setFilter:filter success:^(NSString *filterId) {
                    MXStrongifyAndReturnIfNil(self);
    ...
    {
        operation = [self.sendingnetworkRestClient setFilter:filter success:success failure:failure];
    }
                               ...
                               failure:(void (^)(NSError *error))failure
{
    ...
    if (self.store)
    {
        ...
        [self.store filterWithFilterId:filterId success:^(MXFilterJSONModel * _Nullable filter) {
            ...
            else
            {
                ...
                MXWeakify(self);
                MXHTTPOperation *operation2 = [self.sendingnetworkRestClient getFilterWithFilterId:filterId success:^(MXFilterJSONModel *filter) {
                    MXStrongifyAndReturnIfNil(self);
    ...
    {
         operation = [sendingnetworkRestClient getFilterWithFilterId:filterId success:success failure:failure];
    }
...
- (NSArray<NSString *> *)publicisedGroupsForUser:(NSString*)userId
{
    ...
    if (userId)
    {
        ...
        {
            MXWeakify(self);
            [self.sendingnetworkRestClient getPublicisedGroupsForUsers:@[userId] success:^(NSDictionary<NSString *,NSArray<NSString *> *> *updatedPublicisedGroupsByUserId) {
                MXStrongifyAndReturnIfNil(self);
