                ...
                andDefaultCredentials:(MXCredentials*)defaultCredentials
{
    ...
    if (self)
    {
        ...
        _guideState = loginResponse.guideState;
        ...
        if (!_homeServer)
        {
            ...
            NSString *serverName = [MXTools serverNameInSendingnetworkIdentifier:_userId];
            if (serverName)
            {
        ...
        //FIXME: yk future fix in service port
        if (!_identityServer) {
            _identityServer = @"https://vector.im";
        }
        [self registerRestClientWillRefreshTokensNotification];
    }
    return self;
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    credentials.guideState = [_guideState copyWithZone:zone];
