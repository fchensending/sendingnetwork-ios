...
+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    ...
    if (loginResponse)
    {
        ...
        MXJSONModelSetNumber(loginResponse.guideState, JSONDictionary[@"guide_state"]);
...
@implementation MXScanQRCodeResponse

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXScanQRCodeResponse *response = [[MXScanQRCodeResponse alloc]init];
    if  (response)
    {
        if (JSONDictionary[@"code"]) {
            NSUInteger code = 200;
            MXJSONModelSetUInteger(code, JSONDictionary[@"code"]);
            response.code = (MXScanQRCode)code;
        }
        MXJSONModelSetString(response.ticket, JSONDictionary[@"ticket"]);
        MXJSONModelSetString(response.msg, JSONDictionary[@"msg"]);
    }
    return response;
}
...
@implementation MXAuthQRCodeResponse

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXAuthQRCodeResponse *response = [[MXAuthQRCodeResponse alloc]init];
    if  (response)
    {
        if (JSONDictionary[@"code"]) {
            NSUInteger code = 200;
            MXJSONModelSetUInteger(code, JSONDictionary[@"code"]);
            response.code = (MXAuthQRCode)code;
        }
        MXJSONModelSetString(response.msg, JSONDictionary[@"msg"]);
    }
    return response;
...
+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    ...
    if (openIdToken)
    {
        ...
        MXJSONModelSetString(openIdToken.sendingnetworkServerName, JSONDictionary[@"sendingnetwork_server_name"]);
...
@implementation MXContactVerifyResponse

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXContactVerifyResponse *contactVerifyResponse = [[MXContactVerifyResponse alloc] init];
    if (contactVerifyResponse)
    {
        MXJSONModelSetBoolean(contactVerifyResponse.isFavorite, JSONDictionary[@"is_favorite"]);
    }
...
+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    ...
    if (versionCapabilities)
    {
        ...
        MXJSONModelSetDictionary(roomCapabilitiesData, JSONDictionary[@"org.sendingnetwork.msc3244.room_capabilities"]);
...
@implementation MXRemark

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXRemark *remark = [[MXRemark alloc]init];
    MXJSONModelSetString(remark.name,JSONDictionary[@"name"])
    return remark;
}
...
@implementation MXRemarkListResponse

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXRemarkListResponse *remarkListResponse = [[MXRemarkListResponse alloc]init];
    NSMutableDictionary *dictionary = [NSMutableDictionary dictionary];
    NSDictionary *dic = JSONDictionary[@"remarked_user"];
    if (!dic) {
        return remarkListResponse;
    }
    for (NSString * key in dic.allKeys) {
        MXRemark *remark = [MXRemark modelFromJSON:dic[key]];
        [dictionary setValue:remark forKey:key];
    }
    remarkListResponse.remarkList = dictionary.copy;
    return remarkListResponse;
}

@end

@implementation MXMetrics
+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXMetrics *metrics = [[MXMetrics alloc]init];
    MXJSONModelSetString(metrics.nftsWorth, JSONDictionary[@"est_holding_value_usd"]);
    MXJSONModelSetNumber(metrics.nftNum, JSONDictionary[@"nft_num"]);
    MXJSONModelSetNumber(metrics.collectionNum, JSONDictionary[@"collection_num"]);
    return metrics;
}
@end

@implementation MXPortfolio
+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXPortfolio *porfolio = [[MXPortfolio alloc]init];
    MXJSONModelSetString(porfolio.imageUrl, JSONDictionary[@"nft"][@"image"]);
    if ([porfolio.imageUrl hasSuffix:@".gif"]) {
        return nil;
    }
    return porfolio;
}
...
@implementation MXCryptoModel
+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXCryptoModel *cropto = [[MXCryptoModel alloc]init];
    MXJSONModelSetArray(cropto.images, JSONDictionary[@"displayProps"][@"images"]);
    MXJSONModelSetString(cropto.name, JSONDictionary[@"symbol"]);
    MXJSONModelSetNumber(cropto.price, JSONDictionary[@"price"]);
    MXJSONModelSetString(cropto.quantity, JSONDictionary[@"balance"]);
    MXJSONModelSetString(cropto.amount, JSONDictionary[@"balanceUSD"]);
    return cropto;
}
...
@implementation MXUserAsset
+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXUserAsset *asset = [[MXUserAsset alloc]init];
    MXJSONModelSetString(asset.netWorth, JSONDictionary[@"net_worth"]);
    MXJSONModelSetString(asset.tokensWorth, JSONDictionary[@"tokens_worth"]);
    MXJSONModelSetMXJSONModel(asset.metrics, MXMetrics,JSONDictionary[@"metrics"]);
    asset.nftsWorth = asset.metrics.nftsWorth;
    MXJSONModelSetMXJSONModelArray(asset.portfolios, MXPortfolio, JSONDictionary[@"portfolio"][@"assets"]);
    MXJSONModelSetMXJSONModelArray(asset.cryptos, MXCryptoModel,JSONDictionary[@"assets"]);
    return asset;
}
...
@implementation MXWalletAddress

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXWalletAddress *address = [[MXWalletAddress alloc]init];
    NSArray *array = JSONDictionary[@"data"];
    if ([array isKindOfClass:NSArray.class] && array.count > 0) {
        NSDictionary *dic = array.firstObject;
        MXJSONModelSetString(address.userId, dic[@"user_id"]);
        MXJSONModelSetString(address.walletAddress, dic[@"wallet_address"]);
        MXJSONModelSetString(address.blockChain, dic[@"blockchain"]);
        MXJSONModelSetString(address.netWork, dic[@"network"]);
    }
    return address;
}
...
@implementation MXNFTAsset
+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXNFTAsset *asset = [[MXNFTAsset alloc]init];
    NSDictionary *dic = JSONDictionary[@"nft"];
    MXJSONModelSetString(asset.name, dic[@"name"]);
    MXJSONModelSetString(asset.address, dic[@"contract_address"]);
    MXJSONModelSetString(asset.url, dic[@"image"]);
    MXJSONModelSetString(asset.contractAddress, dic[@"contract_address"]);
    MXJSONModelSetString(asset.tokenId, dic[@"token_id"]);
    if ([asset.url hasSuffix:@".gif"]) {
        return nil;
    }
    return asset;
}
...
@implementation MXNFTResponse

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXNFTResponse *response = [[MXNFTResponse alloc]init];
    MXJSONModelSetNumber(response.total, JSONDictionary[@"name"]);
    MXJSONModelSetMXJSONModelArray(response.assets, MXNFTAsset, JSONDictionary[@"assets"]);
    return response;
}
...
#pragma mark DID
@implementation MXDIDListResponse

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXDIDListResponse *response = [[MXDIDListResponse alloc] init];
    MXJSONModelSetArray(response.array, JSONDictionary[@"data"]);
    return response;
}

@end

@implementation MXDiDInfoResponse

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXDiDInfoResponse *response = [[MXDiDInfoResponse alloc] init];
...
@implementation MXDIDListInfoResponse
...
@implementation MXCreateDIDResponse

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXCreateDIDResponse *response = [[MXCreateDIDResponse alloc] init];
...
@implementation MXSaveDIDResponse

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXSaveDIDResponse *response = [[MXSaveDIDResponse alloc] init];
    MXJSONModelSetString(response.message, JSONDictionary[@"message"]);
    return response;
}
...
@implementation MXUpdateDIDResponse

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXUpdateDIDResponse  *response = [[MXUpdateDIDResponse alloc] init];
    MXJSONModelSetString(response.did, JSONDictionary[@"did"]);
    MXJSONModelSetString(response.message, JSONDictionary[@"message"]);
    MXJSONModelSetString(response.updated, JSONDictionary[@"updated"]);
    return response;
...
@implementation MXPreLoginResponse

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXPreLoginResponse  *response = [[MXPreLoginResponse alloc] init];
    MXJSONModelSetString(response.did, JSONDictionary[@"did"]);
    MXJSONModelSetString(response.message, JSONDictionary[@"message"]);
    MXJSONModelSetString(response.updated, JSONDictionary[@"updated"]);
...
@implementation MXDIDLoginResponse

+ (id)modelFromJSON:(NSDictionary *)JSONDictionary
{
    MXDIDLoginResponse  *response = [[MXDIDLoginResponse alloc] init];
    MXJSONModelSetString(response.accessToken, JSONDictionary[@"access_token"]);
    MXJSONModelSetString(response.userId, JSONDictionary[@"user_id"]);
    MXJSONModelSetString(response.homeServer, JSONDictionary[@"home_server"]);
    MXJSONModelSetString(response.deviceId, JSONDictionary[@"device_id"]);
    MXJSONModelSetNumber(response.guideState, JSONDictionary[@"guide_state"]);
    return response;
}
@end
