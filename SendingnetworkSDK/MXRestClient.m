 ...
 */
NSString *const kMXAPIPrefixPathR0 = @"_api/client/r0";
NSString *const kMXAPIPrefixPathV1 = @"_api/client/v1";
NSString *const kMXAPIPrefixPathV3 = @"_api/client/v3";
NSString *const kMXAPIPrefixPathUnstable = @"_api/client/unstable";
NSString *const kMXAPIPrefixPathUnstableV2 = @"_api/client/unstable/org.sendingnetwork.msc2946";
...
NSString *const kMXAccountDataTypeRemarkUserList = @"m.remarked_user_list";
...
- (NSString *)homeserverSuffix
{
    ...
    if (components.count > 1)
    {
        ...
        NSString *sendingnetworkId = components.firstObject;
        NSRange range = NSMakeRange(0, sendingnetworkId.length);
...
- (MXHTTPOperation*)supportedSendingnetworkVersions:(void (^)(MXSendingnetworkVersions *sendingnetworkVersions))success
                        failure:(void (^)(NSError *error))failure
{
    // There is no versioning in the path of this API
    NSString *path = @"_api/client/versions";
    ...
    return [httpClient requestWithMethod:@"GET"
                              ...
                              parameters:nil
                                 success:^(NSDictionary *JSONResponse) {
                                     ...
                                     if (success)
                                     {
                                         __block MXSendingnetworkVersions *sendingnetworkVersions;
                                         [self dispatchProcessing:^{
                                             MXJSONModelSetMXJSONModel(sendingnetworkVersions, MXSendingnetworkVersions, JSONResponse);
                                         } andCompletion:^{
                                             success(sendingnetworkVersions);
                                         }];
                                     }
                                 }
                                 failure:^(NSError *error) {
...
- (MXHTTPOperation*)wellKnow:(void (^)(MXWellKnown *wellKnown))success
                     failure:(void (^)(NSError *error))failure
{
    NSString *path = @".well-known/api/client";
...
- (MXHTTPOperation *)testUserRegistration:(NSString *)username callback:(void (^)(MXError *mxError))callback
{
    ...
    return [self registerWithParameters:@{@"username": username}
                                ...
                                failure:^(NSError *error)
            {
                // Retrieve the sendingnetwork error back
                MXError *mxError = [[MXError alloc] initWithNSError:error];
                callback(mxError);
            }];
}
...
- (NSString*)registerFallback;
{
    ...
    {
        registerFallback = [[NSURL URLWithString:@"_api/static/client/register/" relativeToURL:[NSURL URLWithString:self.credentials.homeServer]] absoluteString];
    }
...
- (NSString*)loginFallback;
{
    ...
    if (self.credentials.homeServer)
    {
        loginFallback = [[NSURL URLWithString:@"/_api/static/client/login/" relativeToURL:[NSURL URLWithString:self.credentials.homeServer]] absoluteString];
    }
...
#pragma mark - QR Code operations for confirm lgin in web

- (MXHTTPOperation *)scanQRCode:(NSString *)qrCodeId success:(void (^)(MXScanQRCodeResponse *JSONResponse))success failure:(void (^)(NSError *))failure
{
    NSString *path = [NSString stringWithFormat:@"%@/login/scanQRCode", apiPathPrefix];
    NSData *payloadData = [NSJSONSerialization dataWithJSONObject:@{@"qr_code_id": qrCodeId} options:0 error:nil];
    MXWeakify(self);
    return [httpClient requestWithMethod:@"POST"
                                         path:path
                                   parameters:nil
                               ...
                               uploadProgress:nil
                                      success:^(NSDictionary *JSONResponse) {
                                        ...
                                        MXScanQRCodeResponse *response;
                                        MXJSONModelSetMXJSONModel(response, MXScanQRCodeResponse, JSONResponse);
                                          if (success)
                                          {
                                              [self dispatchProcessing:nil andCompletion:^{
                                                  success(response);
...
- (MXHTTPOperation *)authQRCode:(NSString *)qrCodeId
                         ticket:(NSString *)ticket
                        confirmed:(BOOL) confirmed
                        success:(void (^)(MXAuthQRCodeResponse *JSONResponse))success
                        failure:(void (^)(NSError *))failure
{
    NSString *path = [NSString stringWithFormat:@"%@/login/authQRCode", apiPathPrefix];
    NSData *payloadData = [NSJSONSerialization dataWithJSONObject:@{@"qr_code_id": qrCodeId,
                                                                    @"ticket": ticket,
                                                                    @"confirmed": @(confirmed)
                                                                  }
                                                          options:0
    ...
    return [httpClient requestWithMethod:@"POST"
                               ...
                               uploadProgress:nil
                                      success:^(NSDictionary *JSONResponse) {
                                        ...
                                        MXAuthQRCodeResponse *response;
                                        MXJSONModelSetMXJSONModel(response, MXAuthQRCodeResponse, JSONResponse);
                                          if (success)
                                          {
                                              [self dispatchProcessing:nil andCompletion:^{
                                                  success(response);
...
- (MXHTTPOperation*)verifyRegistrationToken:(NSString *)token
                                   session:(NSString *)session
                                   success:(void (^)(void))success
                                    failure:(void (^)(NSError *error))failure{
    MXWeakify(self);
    return [httpClient requestWithMethod:@"POST"
                                    path:[NSString stringWithFormat:@"%@/auth/org.sendingnetwork.msc3231.login.registration_token/fallback/web?token=%@&session=%@", apiPathPrefix, token, session]
                              parameters:nil
                                 success:^(NSDictionary *JSONResponse) {
                                     MXStrongifyAndReturnIfNil(self);
                                     [self dispatchSuccess:success];
                                     ...
                                     failure:(void (^)(NSError *error))failure
{
    ...
    MXWeakify(self);
    operation = [self supportedSendingnetworkVersions:^(MXSendingnetworkVersions *sendingnetworkVersions) {
        MXStrongifyAndReturnIfNil(self);

        if (!sendingnetworkVersions.doesServerSupportSeparateAddAndBind && self.identityServer)
        {
                                               ...
                                               failure:(void (^)(NSError *error))failure
{
    ...
    operation = [self supportedSendingnetworkVersions:^(MXSendingnetworkVersions *sendingnetworkVersions) {
        ...
        if (sendingnetworkVersions.doesServerAcceptIdentityAccessToken)
        {
...
{
    // Define an absolute path based on Sendingnetwork content respository path instead of the base url
...
- (MXHTTPOperation*)fetchVerifyAppsSuccess:(void (^)(NSArray<MXDappInfo*>* apps))success
                                   failure:(void (^)(NSError *error))failure
{
    MXWeakify(self);
    return [httpClient requestWithMethod:@"GET"
                                    path:[NSString stringWithFormat:@"%@/preview_url_app", apiPathPrefix]
                              parameters:nil
                                 success:^(NSDictionary *JSONResponse) {
                                     MXStrongifyAndReturnIfNil(self);
                                     if (success)
                                     {
                                         __block NSArray<MXDappInfo*> *apps;
                                         [self dispatchProcessing:^{
                                             MXJSONModelSetMXJSONModelArray(apps, MXDappInfo, JSONResponse);
                                         } andCompletion:^{
                                             success(apps);
                                         }];
                                     }
                       ...
                       failure:(void (^)(NSError *error))failure
{
    ...
    if (fallbackKeys)
    {
        parameters[@"org.sendingnetwork.msc2732.fallback_keys"] = fallbackKeys;
    }
                                           ...
                                           failure:(void (^)(NSError *error))failure
{
    MXWeakify(self);
    return [httpClient requestWithMethod:@"GET"
                                    path:[NSString stringWithFormat:@"%@/%@/org.sendingnetwork.msc2697.v2/dehydrated_device", credentials.homeServer, kMXAPIPrefixPathUnstable]
                              parameters:@{}
                                 success:^(NSDictionary *JSONResponse) {
                                        ...
                                        failure:(void (^)(NSError *error))failure
{
    MXWeakify(self);
    return [httpClient requestWithMethod:@"POST"
                                    path:[NSString stringWithFormat:@"%@/%@/org.sendingnetwork.msc2697.v2/dehydrated_device/claim", credentials.homeServer, kMXAPIPrefixPathUnstable]
                              ...
                              parameters:parameters
                                 ...
                                 failure:^(NSError *error) {
                                     ...
                                     [self dispatchProcessing:^{
                                         ...
                                         MXError *sendingnetworkError = [[MXError alloc] initWithNSError: error];
                                         ...
                                         if (sendingnetworkError && sendingnetworkError.httpResponse.statusCode == 400)
                                         {
                                           ...
                                           failure:(void (^)(NSError *error))failure
{
    ...
    NSString *path = [NSString stringWithFormat:@"%@/rooms/%@/hierarchy?suggested_only=%@%@%@%@",
                      kMXAPIPrefixPathUnstableV2, spaceId, suggestedOnly ? @"true": @"false", limitParam, maxDepthParam, fromParam];
...
- (MXHTTPOperation*)verifyFavoritesWithUserId:(NSString *)userId
                                    contactId:(NSString *)contactId
                                      forType:(BOOL)isRoom
                                      success:(void (^)(MXContactVerifyResponse *contactVerifyResponse))success
                                      failure:(void (^)(NSError *error))failure
{
    NSString *path = [NSString stringWithFormat:@"%@/contact/%@", kMXAPIPrefixPathR0, userId];
    NSDictionary * data = @{@"contact_id":contactId ? contactId : @"" , @"is_room": isRoom ? @1 : @0 };
    ...
    return [httpClient requestWithMethod:@"POST"
                              ...
                              parameters:data
                                 success:^(NSDictionary *JSONResponse) {
                                    ...
                                    if (success)
                                    {
                                        __block MXContactVerifyResponse *contactVerifyResponse;
                                        [self dispatchProcessing:^{
                                            MXJSONModelSetMXJSONModel(contactVerifyResponse, MXContactVerifyResponse, JSONResponse);
                                        } andCompletion:^{
                                            success(contactVerifyResponse);
                                        }];
                                    }
                                 }
                                 failure:^(NSError *error) {
...
- (MXHTTPOperation*)getRoomIdWithContactId:(NSString *)contactId
                                   success:(void (^)(NSString * _Nullable roomId))success
                                   failure:(void (^)(NSError *error))failure
{
    NSString *path = [NSString stringWithFormat:@"%@/find_dm_room/%@", kMXAPIPrefixPathR0, contactId];
    ...
    return [httpClient requestWithMethod:@"GET"
                              ...
                              parameters:@{}
                                 success:^(NSDictionary *JSONResponse) {
                                    MXStrongifyAndReturnIfNil(self);
                                    NSArray *array = JSONResponse[@"dm_rooms"];
                                    NSString *roomId = array.count > 0 ? array.lastObject : nil;
                                    if (success) {
...
- (MXHTTPOperation*)setRemarkWithUserId:(NSString *)userId
                              contactId:(NSString *)contactId
                                remarkName:(NSString *)remarkName
                                       success:(void (^)(void))success
                                       failure:(void (^)(NSError *error))failure
{
    NSString *path = [NSString stringWithFormat:@"%@/user/%@/account_data/m.remarked_user_list", kMXAPIPrefixPathR0, userId];
    
    MXWeakify(self);
    return [httpClient requestWithMethod:@"PUT"
                                    path:path
                              parameters:@{@"remarked_user":@{contactId:@{@"name":remarkName}}}
                                 success:^(NSDictionary *JSONResponse) {
        if (success)
        {
            [self dispatchSuccess:^{
...
- (MXHTTPOperation*)getRemarkListWithContactId:(NSString *)contactId
                                       success:(void (^)(MXRemarkListResponse * remarkResponse))success
                                       failure:(void (^)(NSError *error))failure
{
    NSString *path = [NSString stringWithFormat:@"%@/user/%@/account_data/m.remarked_user_list", kMXAPIPrefixPathR0, contactId];
    ...
    return [httpClient requestWithMethod:@"GET"
        ...
        if (success)
        {
            ...
            __block MXRemarkListResponse *remarkResponse;
            [self dispatchProcessing:^{
                MXJSONModelSetMXJSONModel(remarkResponse, MXRemarkListResponse, JSONResponse);
            } andCompletion:^{
                success(remarkResponse);
            }];
        }
    }failure:^(NSError *error) {
        MXStrongifyAndReturnIfNil(self);
...
/// get user assets
/// @param address wallet address , not userId
/// @param refresh force refresh, maybe need some time
/// @param success success description
/// @param failure failure description
- (MXHTTPOperation*)getUserAssetsWithAddress:(NSString *)address
                                     refresh:(BOOL)refresh
                                     success:(void (^)(MXUserAsset * userAsset))success
                                     failure:(void (^)(NSError *error))failure
{
    NSMutableString *path = [NSString stringWithFormat:@"/_assets?address=%@",address].mutableCopy;
    if (refresh) {
        [path appendString:@"&refresh=true"];
    }
    MXWeakify(self);
    return [httpClient requestWithMethod:@"GET"
                                    path:path
        ...
        if (success)
        {
            __block MXUserAsset *userAsset;
            [self dispatchProcessing:^{
                MXJSONModelSetMXJSONModel(userAsset, MXUserAsset, JSONResponse);
            } andCompletion:^{
                success(userAsset);
            }];
        }
...
/// @param success success description
/// @param failure failure description
- (MXHTTPOperation*)getWalletAddressWithUserId:(NSString *)userId
                                       success:(void (^)(MXWalletAddress *address))success
                                       failure:(void (^)(NSError *error))failure
{
    NSString *path = [NSString stringWithFormat:@"%@/account/3pid/did", kMXAPIPrefixPathR0];
    ...
    return [httpClient requestWithMethod:@"POST"
        ...
        if (success)
        {
            __block MXWalletAddress *address;
            [self dispatchProcessing:^{
                MXJSONModelSetMXJSONModel(address, MXWalletAddress, JSONResponse);
            } andCompletion:^{
                success(address);
            }];
        }
...
- (MXHTTPOperation*)uploadNFTAvatar:(MXNFTAsset *)nftAsset
                            success:(void (^)(NSString *sendingnetworkImageUrl))success
                            failure:(void (^)(NSError *error))failure
{
    NSString *path = [NSString stringWithFormat:@"%@/upload_by_url?url=%@&filename=%@_%@", kMXContentPrefixPath,nftAsset.url,nftAsset.contractAddress,nftAsset.tokenId];
    MXWeakify(self);
    return [httpClient requestWithMethod:@"POST"
                                    path:path
                              parameters:nil
        ...
        {
            [self dispatchSuccess:^{
                success(JSONResponse[@"content_uri"]);
            }];
        }
    }failure:^(NSError *error) {
        MXStrongifyAndReturnIfNil(self);
...
- (MXHTTPOperation*)getNFTSWithAddress:(NSString *)address
                             pageIndex:(NSUInteger)pageIndex
                                success:(void (^)(MXNFTResponse * response))success
                                failure:(void (^)(NSError *error))failure
{
    NSString *path = [NSString stringWithFormat:@"_nftgo?address=%@&data_type=portfolio&offset=%ld&limit=20", address,pageIndex];
    MXWeakify(self);
    return [httpClient requestWithMethod:@"GET"
                                    path:path
                              parameters:nil
        ...
        if (success)
        {
            __block MXNFTResponse * response = [[MXNFTResponse alloc]init];
            [self dispatchProcessing:^{
                MXJSONModelSetMXJSONModel(response, MXNFTResponse, JSONResponse);
            } andCompletion:^{
                success(response);
            }];
        }
...
- (MXHTTPOperation*)getENSListWithsUserId:(NSString *)userId
                                  success:(void (^)(NSArray <NSString *>* list))success
                                   failure:(void (^)(NSError *error))failure
{
    NSString *path = [NSString stringWithFormat:@"%@/profile/%@/ensdomain",kMXAPIPrefixPathR0, userId];
    MXWeakify(self);
    return [httpClient requestWithMethod:@"GET"
                                    path:path
                              parameters:nil
        ...
        if (success)
        {
            ...
            if (JSONResponse[@"name"]) {
                [array addObject:JSONResponse[@"name"]];
            }
            [self dispatchSuccess:^{
                success(array);
            }];
...
- (MXHTTPOperation*)setOnboardingStep:(NSInteger)step
                              success:(dispatch_block_t)success
                              failure:(void (^)(NSError *error))failure
{
    NSString *path = [NSString stringWithFormat:@"%@/account/guide",kMXAPIPrefixPathR0];
    NSMutableDictionary *param = @{}.mutableCopy;
    if (step > 1) {
        [param setValue:[NSNumber numberWithInteger:step] forKey:@"step"];
    }
    MXWeakify(self);
    return [httpClient requestWithMethod:@"POST"
                                    path:path
...
- (MXHTTPOperation *)getDIDList:(NSString *)address
    ...
    NSString *path = [NSString stringWithFormat:@"%@/address/%@",
                      kMXAPIPrefixPathUnstable,address];
    MXWeakify(self);
    return [httpClient requestWithMethod:@"GET" path:path parameters:nil success:^(NSDictionary *JSONResponse) {
        MXStrongifyAndReturnIfNil(self);
        
        if (success)
        {
            __block MXDIDListResponse * response = [[MXDIDListResponse alloc]init];
            [self dispatchProcessing:^{
                MXJSONModelSetMXJSONModel(response, MXDIDListResponse, JSONResponse);
            } andCompletion:^{
                success(response);
            }];
        }
    ...
    return [httpClient requestWithMethod:@"GET"
                              ...
                              parameters:nil
                                 success:^(NSDictionary *JSONResponse) {
                                     ...
                                     if (success)
                                     {
                                         __block MXDIDListInfoResponse * response = [[MXDIDListInfoResponse alloc]init];
                                         [self dispatchProcessing:^{
                                             MXJSONModelSetMXJSONModel(response, MXDIDListInfoResponse, JSONResponse);
                                         } andCompletion:^{
                                             success(response);
                                         }];
                                     }
...
- (MXHTTPOperation *)postCreateDID:(NSString *)address
                              success:(void (^) (MXCreateDIDResponse *response))success
                           failure:(void (^)(NSError *error))failure {
    NSString *path = [NSString stringWithFormat:@"%@/did/create",
                      kMXAPIPrefixPathUnstable];
    MXWeakify(self);
    return [httpClient requestWithMethod:@"POST"
                                    path:path
                              parameters:@{@"address":address}
                                 success:^(NSDictionary *JSONResponse) {
                                     ...
                                     if (success)
                                     {
                                         __block MXCreateDIDResponse * response = [[MXCreateDIDResponse alloc]init];
                                         [self dispatchProcessing:^{
                                             MXJSONModelSetMXJSONModel(response, MXCreateDIDResponse, JSONResponse);
                                         } andCompletion:^{
                                             success(response);
                                         }];
                                     }
...
- (MXHTTPOperation *)postSaveDID:(NSString *)did
                     withParameter:(NSDictionary *)parameter
                              success:(void (^) (MXSaveDIDResponse *response))success
                         failure:(void (^)(NSError *error))failure {
    NSString *path = [NSString stringWithFormat:@"%@/did/%@",
    ...
    return [httpClient requestWithMethod:@"POST"
                              ...
                              parameters:parameter
                                 success:^(NSDictionary *JSONResponse) {
                                     ...
                                     if (success)
                                     {
                                         __block MXSaveDIDResponse * response = [[MXSaveDIDResponse alloc]init];
                                         [self dispatchProcessing:^{
                                             MXJSONModelSetMXJSONModel(response, MXSaveDIDResponse, JSONResponse);
                                         } andCompletion:^{
                                             success(response);
                                         }];
                                     }
...
- (MXHTTPOperation *)postUpdateDID:(NSString *)did
                     withParameter:(NSDictionary *)parameter
                              success:(void (^) (MXUpdateDIDResponse *response))success
                           failure:(void (^)(NSError *error))failure {
    NSString *path = [NSString stringWithFormat:@"%@/did/%@/update",
                      kMXAPIPrefixPathUnstable,did];
    MXWeakify(self);
    return [httpClient requestWithMethod:@"POST"
                                    path:path
                              parameters:parameter
                                 success:^(NSDictionary *JSONResponse) {
                                     ...
                                     if (success)
                                     {
                                         __block MXUpdateDIDResponse * response = [[MXUpdateDIDResponse alloc]init];
                                         [self dispatchProcessing:^{
                                             MXJSONModelSetMXJSONModel(response, MXUpdateDIDResponse, JSONResponse);
                                         } andCompletion:^{
                                             success(response);
                                         }];
                                     }
    ...
    return [httpClient requestWithMethod:@"POST"
                              ...
                              parameters:parameter
                                 success:^(NSDictionary *JSONResponse) {
                                     ...
                                     if (success)
                                     {
                                         __block MXPreLoginResponse * response = [[MXPreLoginResponse alloc]init];
                                         [self dispatchProcessing:^{
                                             MXJSONModelSetMXJSONModel(response, MXPreLoginResponse, JSONResponse);
                                         } andCompletion:^{
                                             success(response);
                                         }];
                                     }
...
- (MXHTTPOperation *)postLoginDID:(NSString *)did
                    withParameter:(NSDictionary *)parameter
                          ...
                          failure:(void (^)(NSError *error))failure {
    NSString *path = [NSString stringWithFormat:@"%@/did/login",
                      kMXAPIPrefixPathUnstable];
    ...
    return [httpClient requestWithMethod:@"POST"
        ...
        if (success)
        {
            __block MXDIDLoginResponse * response = [[MXDIDLoginResponse alloc]init];
            [self dispatchProcessing:^{
                MXJSONModelSetMXJSONModel(response, MXDIDLoginResponse, JSONResponse);
            } andCompletion:^{
                success(response);
            }];
        }
