...
FOUNDATION_EXPORT NSString *const kMXAccountDataTypeRemarkUserList;
...
/**
 `MXRestClient` makes requests to Sendingnetwork servers.

 It is the single point to send requests to Sendingnetwork servers which are:
    - the specified Sendingnetwork home server
    - the Sendingnetwork content repository manage by this home server
    - the specified Sendingnetwork identity server
 */
...
/**
 Credentials for the Sendingnetwork Client-Server API.
 */
...
/**
 The homeserver suffix (for example ":sending.network"). Available only when credentials have been set.
 */
...
/**
 ...
 By default, it is '_api/client/r0'. See kMXAPIPrefixPathR0 and kMXAPIPrefixPathUnstable for constants.
 */
@property (nonatomic) NSString *apiPathPrefix;

/**
 The Sendingnetwork content repository prefix to use.
...
/**
 Create an instance based on a sendingnetwork user account.
...
/**
 ...
 As described in http://sending.network/docs/spec/client_server/r0.2.0.html#client-authentication some registration flows require to
...
/**
 ...
 This method manages the full flow for simple login types and returns the credentials of the newly created sendingnetwork user.
 ...
 @param username the user id (ex: "@bob:sending.network") or the user id localpart (ex: "bob") of the user to register. Can be nil.
...
/**
 ...
 Check MXSendingnetworkVersion.doesServerRequireIdentityServerParam to determine if
...
/**
 ...
 This method manages the full flow for simple login types and returns the credentials of the logged sendingnetwork user.
 ...
 @param username the user id (ex: "@bob:sending.network") or the user id localpart (ex: "bob") of the user to register.
 @param password his password.
 @param success A block object called when the operation succeeds. It provides credentials to use to create a MXRestClient.
 @param failure A block object called when the operation fails.
...
/// Scan QR code generated by web client
///
/// @param qrCodeId qrCodeId generated by web client
/// @param success  A block object called when the operation succeeds.
/// @param failure  A block object called when the operation fails.
///
///  @return a MXHTTPOperation instance.
- (MXHTTPOperation *)scanQRCode:(NSString *)qrCodeId
                        success:(void (^)(MXScanQRCodeResponse *JSONResponse))success
                        failure:(void (^)(NSError *))failure
                               NS_REFINED_FOR_SWIFT;

/// Auth QR code generated by web client
///
/// @param qrCodeId qrCodeId generated by web client
/// @param ticket  ticket from scanQRCode's response
/// @param success  A block object called when the operation succeeds.
/// @param failure  A block object called when the operation fails.
///
///  @return a MXHTTPOperation instance.
...
/**
  Verify the register token and session, so the session can use authorization.
 
 https://confluence.sding.me/pages/viewpage.action?pageId=7212767
 
 @param token Active code.
 @param session Pengding session which need be verified with active code.
 @param success A block object called when the operation succeeds.
 @param failure A block object called when the operation fails.
...
/**
 ...
 @param joinRule the rule to set.
 @param roomId the id of the room.
 @param allowedParentIds Optional: list of allowedParentIds (required only for `restricted` join rule as per [MSC3083](https://github.com/sending-network/sendingnetwork-doc/pull/3083) )
 @param success A block object called when the operation succeeds.
 @param failure A block object called when the operation fails.
...
/**
 ...
 @param roomId the id of the room.
 @param success A block object called when the operation succeeds. It provides the room enhanced join rule as per [MSC3083](https://github.com/sending-network/sendingnetwork-doc/pull/3083.
 @param failure A block object called when the operation fails.
...
/**
 ...
 @param parameters the parameters. Refer to the sendingnetwork specification for details.
...
/**
 ...
 home server JSON response. @see http://sending.network/docs/api/client-server/#!/-rooms/get_state_events
 @param failure A block object called when the operation fails.
...
/**
 ...
 the homeserver JSON response. @see http://sending.network/docs/api/client-server/#!/-rooms/get_room_sync_data
 @param failure A block object called when the operation fails.
...
/**
 ...
 @param via servers, that should be tried to request a summary from, if it can't be generated locally. These can be from a sendingnetwork URI, sendingnetwork.to link or a `m.space.child` event for example.
...
/**
 Link an authenticated 3rd party id to the Sendingnetwork user.
 ...
 to the account's Sendingnetwork ID with the identity server.
...
/**
 Remove a 3rd party id from the Sendingnetwork user information.
...
/**
 List all 3PIDs linked to the Sendingnetwork user account.
...
/**
 Retrieve a Sendingnetwork User ID linked to a user on the third party service, given a set of user parameters.
...
/**
 Get verified app links

 @param success A block object called when the operation succeeds. It provides an `NSArray<MXDappInfo*>` object
 @param failure A block object called when the operation fails.
...
/**
 ...
 @param mxcContentURI the Sendingnetwork content URI to scan (in the form of "mxc://...").
...
/**
 ...
 @param parameters the search parameters as defined by the Sendingnetwork search spec (http://sending.network/docs/api/client-server/#!/Search/post_search ).
 @param nextBatch the token to pass for doing pagination from a previous response.
...
- (MXHTTPOperation*)getENSListWithsUserId:(NSString *)userId
                                  ...
                                  failure:(void (^)(NSError *error))failure;
/// Onboarding setp record
/// @param step if step == 1 , Means the step is complete ，if step > 1 ,is your User-defined status
